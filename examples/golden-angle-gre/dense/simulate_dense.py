...
params={'return_type':'mat','gpu':False,'precision':'f64','Δt_rf':5e-6,'Δt':10e-6}
for ss in [4,2]:
 ...
 np.savez_compressed(O/f'signal_ss{ss}.npz',signal=raw,**fields)
 report=dict(ss=ss,spins=len(kw['x']),in_plane_spins=len(fields['x']),dx_mm=dx*1000,dy_mm=dy*1000,z_positions=33,z_step_mm=dz*1000,seconds=time.time()-start,rf_step_us=5,gradient_step_us=10)
 (O/f'simulation_ss{ss}.json').write_text(json.dumps(report,indent=2))
